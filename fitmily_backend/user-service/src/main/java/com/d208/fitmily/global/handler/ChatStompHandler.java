...
public class ChatStompHandler {
    ...
    public Message<?> handle(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT.equals(command)) {
            System.out.println("🔤 [CHAT CONNECT] 요청 도착");
            ...
            if (authHeader != null && authHeader.startsWith("Bearer ")) {
                ...
                System.out.println("✅ 채팅 토큰 인증 성공 → userId: " + userId + ", role: " + role);
                ...
                System.out.println("🔐 채팅 setUser(auth) 설정 완료 → " + auth.getName());
                System.out.println("💾 채팅 sessionAttributes 저장 완료: sessionId = " + accessor.getSessionId());
            }
        }
        ...
        // SEND / SUBSCRIBE 시
            ...
            if (accessor.getUser() == null) {
                System.out.println("🧩 [CHAT SEND/SUB] getUser() == null → 세션에서 복원 시도");
                ...
                if (authObj instanceof Authentication auth) {
                    accessor.setUser(auth);
                    SecurityContextHolder.getContext().setAuthentication(auth);
                    System.out.println("✅ 채팅 인증 정보 복원 성공 → userId: " + auth.getName());
                } else {
                    System.out.println("❌ 채팅 인증 정보 복원 실패 → 세션에 없음");
                }
            } else {
                System.out.println("✅ 채팅 getUser() 이미 존재: " + accessor.getUser().getName());
